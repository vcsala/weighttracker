...
#define DEFAULT_ACTIVE_TEXT_COLOR GColorBlack
#define DEFAULT_INACTIVE_TEXT_COLOR PBL_IF_COLOR_ELSE(GColorBlack, GColorWhite)
...
static int prv_get_y_offset_which_vertically_centers_font(int height) {
  ...
  for (int i = 0, current_x_offset = 0; i < data->num_cells; i++) {
    if (data->callbacks.get_cell_text) {
      ...
      if (text) {
        ...
        int y_offset = prv_get_y_offset_which_vertically_centers_font(height);

        if (data->selected_cell_idx == i && data->bump_is_upwards) {
          y_offset -= prv_get_pixels_for_bump_settle(data->bump_settle_anim_progress);
        }

        GColor fg_color = data->inactive_foreground_color;

        if (data->selected_cell_idx == i) {
          int delta = (data->bump_text_anim_progress * FONT_TOP_PADDING) / 100;
          if (data->bump_is_upwards) {
            delta *= -1;
          }
          y_offset += delta;
          fg_color = data->active_foreground_color;
        }

        graphics_context_set_text_color(ctx, fg_color);
        GRect rect = GRect(current_x_offset, y_offset, data->cell_widths[i], height);
        graphics_draw_text(ctx, text, fonts_get_system_font(FONT_KEY_GOTHIC_28_BOLD), rect, GTextOverflowModeFill, GTextAlignmentCenter, NULL);
      }
    }
...
static Layer* selection_layer_init(SelectionLayerData *selection_layer_, GRect frame, int num_cells) {
  ...
  *selection_layer_data = (SelectionLayerData) {
    ...
    .active_foreground_color = DEFAULT_ACTIVE_TEXT_COLOR,
    .inactive_foreground_color = DEFAULT_INACTIVE_TEXT_COLOR,
    .num_cells = num_cells,
...
void selection_layer_set_inactive_fg_color(Layer *layer, GColor color) {
  SelectionLayerData *data = layer_get_data(layer);

  if (data) {
    data->inactive_foreground_color = color;
  }
}

void selection_layer_set_active_fg_color(Layer *layer, GColor color) {
  SelectionLayerData *data = layer_get_data(layer);

  if (data) {
    data->active_foreground_color = color;
  }
}
...
void selection_layer_set_selected(Layer *layer, int index) {
  SelectionLayerData *data = layer_get_data(layer);

  if (data) {
    if (index >= 0 && index < data->num_cells) {
      data->selected_cell_idx = index;
    }
    layer_mark_dirty(layer);
  }
}

void selection_layer_set_click_config_onto_actionbar(Layer *layer, ActionBarLayer *actionbarlayer) {
  if (layer && actionbarlayer) {
    action_bar_layer_set_click_config_provider(actionbarlayer, (ClickConfigProvider)prv_click_config_provider);
    action_bar_layer_set_context(actionbarlayer, layer);
  }
}
